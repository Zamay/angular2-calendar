...
export class WeeksComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // this.day =  this.Months[valueState['months'].number];  // это месяц .. почему то day у него имя ?!
  ...
  public showNote(month) {
    ...
    // this.daysOfMonth = [];
    // let dayOfMonth = {};
    // for (let item = 0; item < this.month.length; item++) {
    //   for (const i of this.month[item]) {
    //     const notes = this.localStorageSer.getLocalStorage(['', months, i.day, i.year]);
    //     if (notes !== null) {
    //       this.showNotes = {
    //         day: i.day,
    //         months: this.day[1],
